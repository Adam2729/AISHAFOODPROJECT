...
- Start from `/admin/access` for browser workflows. If you need a temporary API fallback, send `x-admin-key: <ADMIN_KEY>`.
- ON: `POST /api/admin/maintenance` body `{ "enabled": true }`
- OFF: `POST /api/admin/maintenance` body `{ "enabled": false }`
