...
export function buildUpsellSuggestions(args: BuildSuggestionsArgs): Suggestion[] {
  ...
  if (onlyMainsInCart) {
    ...
    if (drink) {
      suggestions.push({
        ...
        reasonFr: "Ajoute une boisson pour completer ta commande.",
      });
    }
  }
  ...
  if (onlyStaplesInCart) {
    ...
    if (essential) {
      suggestions.push({
        ...
        reasonFr: "Il te manque peut-etre un indispensable pour tes achats.",
      });
    }
  }
  ...
  if (subtotal < 400) {
    while (suggestions.length < 2) {
      ...
      suggestions.push({
        ...
        reasonFr: "Complete ta commande avec une option economique.",
      });
    }
  }
  ...
  while (suggestions.length < 2) {
    ...
    suggestions.push({
      ...
      reasonFr: "Suggere pour completer tes achats.",
    });
  }
