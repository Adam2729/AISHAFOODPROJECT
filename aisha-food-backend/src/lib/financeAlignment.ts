...
export async function computeFinanceAlignmentForWeek(
  ...
  const [deliveredAggRows, settlementRows, cashRows, adjustmentAggRows] = await Promise.all([
    ...
    Order.aggregate<AdjustmentAggRow>([
      {
        $match: {
          ...matchWithBusiness,
          "settlement.weekKey": normalizedWeekKey,
      ...
      { $unwind: "$adjustments" },
      {
        $group: {
          ...
          adjustmentTotal: { $sum: { $ifNull: ["$adjustments.amount", 0] } },
        },
      },
    ]),
  ]);
  ...
  const adjustmentsByBusiness = new Map<
    string,
    {
      count: number;
      total: number;
  ...
  >();
  for (const row of adjustmentAggRows) {
    adjustmentsByBusiness.set(String(row._id), {
      count: Math.max(0, Math.round(asNumber(row.adjustmentCount, 0))),
      total: normalizeMoney(row.adjustmentTotal),
    });
  }
  ...
  for (const businessId of businessIds) {
    ...
    const adjustments = adjustmentsByBusiness.get(businessId) || {
      count: 0,
      total: 0,
    };
