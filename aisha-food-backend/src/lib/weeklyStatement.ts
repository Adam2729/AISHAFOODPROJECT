...
type StatementCurrency = "DOP" | "XOF";
...
type CashCollectionRow = {
  ...
  driverCash?: {
    driverCollectedTotalRdp?: number | null;
    driverHandedTotalRdp?: number | null;
    driverDisputedTotalRdp?: number | null;
    merchantCashReceivedTotalRdp?: number | null;
    mismatchSignal?: boolean | null;
  } | null;
...
function normalizeStatementCurrency(value: unknown): StatementCurrency {
...
export async function computeWeeklyStatementPack(businessId: string, weekKey: string) {
  ...
  const [orders, settlement, cashCollection, business] = await Promise.all([
    Order.find({
      ...
      .select(
        "_id orderNumber currency createdAt updatedAt status subtotal total commissionAmount deliveryFeeToCustomer deliverySnapshot.mode discount sla.deliveredAt"
      )
      .sort({ createdAt: 1 })
      .lean<OrderRow[]>(),
    Settlement.findOne({
    ...
    CashCollection.findOne({
      ...
      .select(
        "businessId businessName weekKey status expected.netSubtotal reported.cashCollected reported.collectorName reported.collectionMethod reported.receiptPhotoUrl reported.receiptRef reported.reportedAt integrity.expectedHash driverCash.driverCollectedTotalRdp driverCash.driverHandedTotalRdp driverCash.driverDisputedTotalRdp driverCash.merchantCashReceivedTotalRdp driverCash.mismatchSignal submittedAt verifiedAt"
      )
      .lean<CashCollectionRow | null>(),
    Business.findById(businessObjectId).select("_id name").lean<BusinessRow | null>(),
  ]);

  const orderObjectIds = orders.map((row) => row._id).filter(Boolean);
  const payoutRows = orderObjectIds.length
    ? await RiderPayout.find({ orderId: { $in: orderObjectIds } })
        .select("orderId amount deliveryFeeCharged platformMargin")
        .lean<RiderPayoutRow[]>()
    : [];
  const payoutByOrderId = new Map(
    payoutRows.map((row) => [
      String(row.orderId),
      {
        amount: roundCurrency(safeNumber(row.amount)),
        deliveryFeeCharged: roundCurrency(safeNumber(row.deliveryFeeCharged)),
        platformMargin: roundCurrency(safeNumber(row.platformMargin)),
      },
    ])
  );
  ...
  const mappedOrders = orders.map((row) => {
    ...
    const deliveryFee = roundCurrency(safeNumber(row.deliveryFeeToCustomer));
    const orderTotal = roundCurrency(row.total != null ? safeNumber(row.total) : subtotal);
    const deliveryMode = normalizeDeliveryMode(row.deliverySnapshot?.mode, "self_delivery");
    const payout = payoutByOrderId.get(String(row._id));
    ...
    return {
      ...
      deliveryFee,
      deliveryMode,
      driverPayoutAmount: roundCurrency(safeNumber(payout?.amount)),
      platformDeliveryMargin: roundCurrency(safeNumber(payout?.platformMargin)),
      commissionAmount,
  ...
  const orderTotal = roundCurrency(
    mappedOrders.reduce((sum, row) => sum + row.orderTotal, 0)
  );
  const deliveryFeeTotal = roundCurrency(
    mappedOrders.reduce((sum, row) => sum + row.deliveryFee, 0)
  );
  const commissionTotal = roundCurrency(
    mappedOrders.reduce((sum, row) => sum + row.commissionAmount, 0)
  );
  const merchantNetAfterCommission = roundCurrency(
    mappedOrders.reduce((sum, row) => sum + (row.subtotal - row.commissionAmount), 0)
  );
  const platformDriverOrdersCount = mappedOrders.filter(
    (row) => row.deliveryMode === "platform_driver"
  ).length;
  const selfDeliveryOrdersCount = Math.max(0, mappedOrders.length - platformDriverOrdersCount);
  const driverPayoutTotal = roundCurrency(
    mappedOrders.reduce((sum, row) => sum + row.driverPayoutAmount, 0)
  );
  const platformDeliveryMarginTotal = roundCurrency(
    mappedOrders.reduce((sum, row) => sum + row.platformDeliveryMargin, 0)
  );
  const platformDriverDeliveryFeeTotal = roundCurrency(
    mappedOrders.reduce(
      (sum, row) => sum + (row.deliveryMode === "platform_driver" ? row.deliveryFee : 0),
      0
    )
  );
  const selfDeliveryDeliveryFeeTotal = roundCurrency(
    mappedOrders.reduce(
      (sum, row) => sum + (row.deliveryMode === "self_delivery" ? row.deliveryFee : 0),
      0
    )
  );
  ...
  const cashVerified =
    cashCollection && (cashCollection.status === "verified" || cashCollection.status === "closed")
      ? cashReported
      : null;
  const variance = cashReported == null ? 0 : roundCurrency(cashReported - cashExpected);

  const driverCashCollectedTotal = roundCurrency(
    safeNumber(cashCollection?.driverCash?.driverCollectedTotalRdp)
  );
  const driverCashHandedTotal = roundCurrency(
    safeNumber(cashCollection?.driverCash?.driverHandedTotalRdp)
  );
  const driverCashDisputedTotal = roundCurrency(
    safeNumber(cashCollection?.driverCash?.driverDisputedTotalRdp)
  );
  const merchantCashReceivedTotal = roundCurrency(
    safeNumber(cashCollection?.driverCash?.merchantCashReceivedTotalRdp)
  );
  ...
  const businessName =
    ...
    "Business";
  const currency = normalizeStatementCurrency(orders[0]?.currency);
  ...
  return {
    ...
    cash: {
      ...
      driverCash: {
        driverCollectedTotal: driverCashCollectedTotal,
        driverHandedTotal: driverCashHandedTotal,
        driverDisputedTotal: driverCashDisputedTotal,
        merchantCashReceivedTotal,
        mismatchSignal: Boolean(cashCollection?.driverCash?.mismatchSignal),
      },
    },
    promos: {
      promoOrdersCount,
