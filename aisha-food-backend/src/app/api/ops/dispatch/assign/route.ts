...
export async function POST(req: Request) {
  try {
    ...
    if (sameObjectId(currentDriverId, driver._id)) {
      return ok({
    ...
    if (currentDriverId) {
      return fail(
        "INVALID_STATE",
        "Order already has an assigned driver. Use the reassign endpoint instead.",
        409
      );
    }
    ...
    if (!updated) {
      ...
      return fail("INVALID_STATE", "Could not assign driver.", 409);
    }
