...
export async function GET(req: Request) {
  try {
    ...
    const rawBusinesses = await Business.find(businessFilter)
      .select(
        "name phone whatsapp address logoUrl location type deliveryType isActive subscription performance paused isManuallyPaused busyUntil hours eta deliveryPolicy"
      )
      .sort({ createdAt: -1 })
      .lean();
