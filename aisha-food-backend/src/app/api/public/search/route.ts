  ...
  try {
    ...
    if (useText) {
      try {
        productCandidates = await Product.find(
          ...
          .select("businessId name category price imageUrl displaySize quantityValue quantityUnit score")
          .sort({ score: { $meta: "textScore" } })
          .limit(80)
          .lean<ProductLean[]>();
      } catch {
    ...
    if (!productCandidates.length) {
      productCandidates = await Product.find({
        ...
        .select("businessId name category price imageUrl displaySize quantityValue quantityUnit")
        .limit(80)
        .lean<ProductLean[]>();
    }
