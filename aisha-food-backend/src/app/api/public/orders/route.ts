...
export async function POST(req: Request) {
  ...
  try {
    ...
    const businessId = normalizeString(body.businessId || body.restaurantId);
    const city = normalizeString(body.city) || String(selectedCity.name || "").trim() || "Active city";
    const notes = normalizeString(body.notes).slice(0, 280);
    const paymentMethodRaw = normalizeString(body.paymentMethod).toLowerCase();
    const items = Array.isArray(body.items)
      ? body.items.map((item) => ({
          productId: normalizeString(item?.productId || item?.itemId),
    ...
    if (!customerName || !phone || !address || !businessId) {
      return finish(
        fail(
          "VALIDATION_ERROR",
          "customerName, phone, address and businessId/restaurantId are required."
        ),
        400
      );
    }
    ...
    if (!coordsProvided) {
      if (!hasGoogleKey) {
        if (allowDevLocationBypass || phase6StylePayload) {
          lat = cityCenter.lat;
          lng = cityCenter.lng;
        } else {
          return finish(
    ...
    if (!citySupportsPaymentMethod(selectedCity, paymentMethod)) {
      return finish(
        fail("PAYMENT_METHOD_NOT_AVAILABLE", "This payment method is not available in the selected city.", 409),
        409,
    ...
    const orderItems = items.map((it) => {
      ...
      return {
        ...
        displaySize: String(p.displaySize || ""),
        quantityValue: p.quantityValue ?? null,
        quantityUnit: String(p.quantityUnit || ""),
        qty,
        unitPrice,
        lineTotal,
      };
    ...
    let discountPromoCodeId: mongoose.Types.ObjectId | null = null;
    ...
    if (promoCodeRaw) {
      const promoCodeDoc = await PromoCode.findOne({
        code: promoCodeRaw,
        cityId: selectedCity._id,
      }).lean<PromoCodeLean | null>();
      if (promoCodeDoc) {
        const promoCodeResult = computePromoCodeDiscount({
          promoCode: promoCodeDoc,
          subtotal: subtotalBefore,
          now: new Date(),
        });
        if (promoCodeResult.discountAmount <= 0) {
          return finish(
            fail("PROMO_INVALID", promoCodeReasonToMessage(promoCodeResult.reason), 400),
            400,
            {
              businessId,
              promoCode: promoCodeRaw,
              reason: promoCodeResult.reason,
            }
          );
        }

        discountSource = "promo";
        discountCode = promoCodeDoc.code;
        discountPromoCodeId = promoCodeDoc._id;
        discountAmount = roundCurrency(promoCodeResult.discountAmount);
        subtotalAfter = roundCurrency(promoCodeResult.finalSubtotal);
      } else {
        ...
        if (promoResult.discountAmount <= 0) {
          return finish(
            fail("PROMO_INVALID", promoReasonToMessage(promoResult.reason), 400),
            400,
            {
              businessId,
              promoCode: promoCodeRaw,
    ...
    if (referralCodeRaw) {
      ...
      if (isNewCustomer) {
        const referrer = await findReferralOwnerByCode({
          cityId: selectedCity._id,
    ...
    for (let attempt = 0; attempt < 5; attempt++) {
      ...
      try {
        created = await Order.create({
          ...
          phone,
          phoneHash,
          sessionIdHash: rateIdentity.sessionIdHash || "",
          address,
          notes,
          ...
          discount: {
            ...
            promoCodeId: discountPromoCodeId,
    ...
    if (discountPromoCodeId) {
      const promoUsageUpdate = await PromoCode.updateOne(
        {
          _id: discountPromoCodeId,
          cityId: selectedCity._id,
          $or: [
            { usageLimit: null },
      ...
      if ((promoUsageUpdate.modifiedCount || 0) < 1) {
        await PaymentEvent.deleteMany({ orderId: created._id }).catch(() => null);
        await Order.deleteOne({ _id: created._id }).catch(() => null);
        return finish(
          fail("PROMO_USAGE_LIMIT_REACHED", "This promo code has reached its usage limit.", 409),
          409,
          {
            businessId,
            promoCode: promoCodeRaw,
    ...
    if (referralSnapshot.referrerPhoneHash && phoneHash && selectedCity._id) {
      await Referral.findOneAndUpdate(
        {
          referredPhoneHash: phoneHash,
          cityId: selectedCity._id,
        },
        {
          $setOnInsert: {
            referrerPhoneHash: referralSnapshot.referrerPhoneHash,
            referredPhoneHash: phoneHash,
            cityId: selectedCity._id,
            rewardAmount: Math.max(0, Number(REFERRAL_REFERRER_BONUS_RDP || 0)),
            status: "pending",
            orderId: null,
          },
        },
      ...
      ok(
        {
          ...
          support: {
            whatsapp: market.supportWhatsApp || SUPPORT_WHATSAPP_E164,
            defaultText:
              market.defaultLanguage === "es"
          ...
          loyalty: {
            pendingPoints: Math.max(0, Math.floor(total / 100)),
            referralCodeUsed: referralSnapshot.usedCode || null,
            referralRewardPending: Boolean(referralSnapshot.usedCode),
          },
          deliveryOtp,
