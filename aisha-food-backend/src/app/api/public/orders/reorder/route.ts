...
export async function POST(req: Request) {
  try {
    ...
    const business = await Business.findById(previous.businessId)
      .select("name type isActive paused location subscription")
      .lean();
    ...
    const products = await Product.find({
      ...
      .select("_id name price category isAvailable isArchived displaySize quantityValue quantityUnit")
      .lean();
    ...
    const removedItems: Array<{
      productId: string;
      name: string;
      reasonCode: "archived" | "unavailable";
      reason: string;
      category: string;
      displaySize: string;
      quantityValue: number | null;
    ...
    for (const item of previousItems) {
      ...
      if (!live || live.isArchived === true || live.isAvailable === false) {
        const reasonCode = live?.isArchived === true ? "archived" : "unavailable";
        removedItems.push({
          productId: id,
          ...
          reason:
            reasonCode === "archived"
              ? "Product is no longer in the active catalog."
              : "Product is currently unavailable.",
          category: String(item?.category || live?.category || ""),
          displaySize: formatProductSizeLabel({
            displaySize: item?.displaySize || live?.displaySize,
            quantityValue: item?.quantityValue ?? live?.quantityValue,
            quantityUnit: item?.quantityUnit || live?.quantityUnit,
          }),
          quantityValue: item?.quantityValue ?? live?.quantityValue ?? null,
          quantityUnit: String(item?.quantityUnit || live?.quantityUnit || ""),
        });
        continue;
      }
      ...
      items.push({
        ...
        displaySize: formatProductSizeLabel({
          displaySize: live.displaySize,
          quantityValue: live.quantityValue,
          quantityUnit: live.quantityUnit,
        }),
        quantityValue: live.quantityValue ?? null,
        quantityUnit: String(live.quantityUnit || ""),
      });
    }
