...
export async function POST(req: Request) {
  ...
  try {
    ...
    if (confirm !== "ACK") {
      return fail("VALIDATION_ERROR", 'confirm must equal "ACK".', 400);
    }
    if (by.length > 60) {
    ...
    if (!updated) {
      const exists = await FinanceAlert.exists({ _id: new mongoose.Types.ObjectId(alertId) });
      if (!exists) return fail("NOT_FOUND", "Finance alert not found.", 404);
      return fail("INVALID_STATE", "Alert cannot be acknowledged from current status.", 409);
    }

    logRequest(req, {
      route: "admin.finance.alerts.ack",
      status: 200,
    ...
    return ok({
      alert: {
        ...
        status: String(updated.status || "acknowledged"),
    ...
    logRequest(req, {
      ...
      durationMs: Date.now() - startedAt,
      extra: {
        message: err.message || "Could not acknowledge finance alert.",
      },
    });
    return fail(
      err.code || "SERVER_ERROR",
      err.message || "Could not acknowledge finance alert.",
      status
    );
  }
}
