...
type OrderLean = DeliveryFinalizationOrder & {
  orderNumber?: string;
...
export async function POST(req: Request) {
  try {
    ...
    const order = await Order.findOne({
      ...
      cityId: new mongoose.Types.ObjectId(tokenCityId),
      ...
      .select(
        [
          ...
          "dispatch.cashCollectedByDriver",
          "deliveryProof.required",
          "deliveryProof.otpHash",
          "deliveryProof.otpCreatedAt",
          "deliveryProof.failedAttempts",
          "deliveryProof.lastFailedAt",
    ...
    if (!order) return fail("NOT_FOUND", "Platform-driver order not assigned to this driver.", 404);
    ...
    const finalization = await finalizeDeliveredOrder({
      order,
      deliveryOtp,
      routeTag: "driver.orders.delivered",
      driverDeliveredConfirmedAt: existingDelivered || now,
    });
    ...
    if (!finalization.idempotent) {
      await settleNotificationWrites(
        "driver.orders.delivered",
        [
          queueOrderDeliveredNotifications({
            orderId: order._id,
            orderNumber: order.orderNumber || null,
            businessId: order.businessId,
            cityId: order.cityId || new mongoose.Types.ObjectId(tokenCityId),
            customerPhoneHash: String(order.phoneHash || "").trim() || null,
            deliveryMode: "platform_driver",
            source: "driver.orders.delivered",
          }),
        ],
        {
          orderId: String(order._id),
    ...
    const updated = await Order.findById(order._id)
      .select("status settlement.counted dispatch.deliveredConfirmedAt dispatch.cashCollectedByDriver")
      .lean<{
        status?: string;
        settlement?: {
          counted?: boolean;
    ...
    return ok({
      ...
      settlementCounted: Boolean(updated?.settlement?.counted),
