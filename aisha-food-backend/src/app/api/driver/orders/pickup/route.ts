...
export async function POST(req: Request) {
  try {
    ...
    const order = await Order.findOne({
      ...
      .select("_id businessId status deliverySnapshot.mode dispatch.pickupConfirmedAt")
      .lean<OrderLean | null>();

    if (!order) return fail("NOT_FOUND", "Platform-driver order not assigned to this driver in selected city.", 404);
