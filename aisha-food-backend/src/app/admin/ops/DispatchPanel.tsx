...
import { getMerchantIssueTypeLabel } from "@/lib/orderOperations";
...
function dispatchStateLabel(order: OrderRow) {
...
export default function DispatchPanel({ adminKey: _adminKey }: Props) {
  void _adminKey;
  ...
  async function loadBoard(nextStatus = statusFilter) {
    ...
    try {
      const res = await fetch(
        `/api/admin/dispatch/board?status=${encodeURIComponent(nextStatus)}`,
        { cache: "no-store" }
      );
  ...
  async function unassignDriver(order: OrderRow) {
    ...
    setSuccess("");
    try {
      const res = await fetch(`/api/admin/dispatch/unassign`, {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({
          orderId: order.orderId,
  ...
  async function saveNote(order: OrderRow) {
    ...
    setSuccess("");
    try {
      const res = await fetch(`/api/admin/dispatch/note`, {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({
          orderId: order.orderId,
...
function OrderCard({
  ...
  const trackHref = order.cityId
    ? `/order/${encodeURIComponent(order.orderId)}?cityId=${encodeURIComponent(order.cityId)}`
  ...
  return (
    <div className={`rounded border p-2 ${isLate ? "border-red-300 bg-red-50" : "border-slate-200"}`}>
      ...
      <div className="mt-1 text-xs text-slate-600">
        Dispatch state: {dispatchStateLabel(order)}
        {order.dispatch?.offerExpiresAt ? ` | offer until ${formatDateTime(order.dispatch.offerExpiresAt)}` : ""}
      </div>
      {order.latestIssue?.type || order.latestIssue?.note ? (
        <div className="mt-1 text-xs text-amber-700">
          Latest issue: {getMerchantIssueTypeLabel(order.latestIssue?.type)}
          {order.latestIssue?.note ? ` - ${order.latestIssue.note}` : ""}
        </div>
      ) : null}
      {order.latestEvent?.label ? (
        <div className="mt-1 text-xs text-slate-500">
          Latest event: {order.latestEvent.label}
          {order.latestEvent.detail ? ` - ${order.latestEvent.detail}` : ""}
        </div>
      ) : null}
