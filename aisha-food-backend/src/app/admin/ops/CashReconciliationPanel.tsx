...
export default function CashReconciliationPanel({
  ...
  async function markDriverCashHanded(row: DriverCashRow) {
    ...
    try {
      const response = await fetch(
        `/api/admin/driver-cash/mark-handed`,
        {
          method: "POST",
  ...
  async function openDriverCashDispute(row: DriverCashRow) {
    ...
    try {
      const response = await fetch(
        `/api/admin/driver-cash/dispute/open`,
        {
          method: "POST",
          headers: { "Content-Type": "application/json" },
          body: JSON.stringify({
  ...
  async function resolveDriverCashDispute(
    ...
    try {
      const response = await fetch(
        `/api/admin/driver-cash/dispute/resolve`,
        {
          method: "POST",
          headers: { "Content-Type": "application/json" },
          body: JSON.stringify({
  ...
  async function runAction(row: Row, action: ActionType) {
    ...
    try {
      const response = await fetch(
        `/api/admin/cash-collections/verify`,
        {
          method: "POST",
          headers: { "Content-Type": "application/json" },
          body: JSON.stringify({
  ...
  async function openAudits(row: Row) {
    ...
    try {
      const response = await fetch(
        `/api/admin/cash-collections/audits?businessId=${encodeURIComponent(row.businessId)}&weekKey=${encodeURIComponent(weekKey)}&limit=20`,
        { cache: "no-store" }
      );
      const json = await response.json().catch(() => null);
      if (!response.ok || !json?.ok) {
  ...
  async function recomputeNow() {
    ...
    try {
      const response = await fetch(
        `/api/admin/jobs/cash-collections-compute?weekKey=${encodeURIComponent(
          weekKey
        )}`,
        {
          method: "POST",
                ...
                <tr key={row.id} className="border-t border-slate-100 align-top">
                  ...
                  <td className="py-2">
                    <div className="flex flex-wrap gap-1">
                      ...
                      <a
                        href={`/admin/statements?businessId=${encodeURIComponent(
                          row.businessId
