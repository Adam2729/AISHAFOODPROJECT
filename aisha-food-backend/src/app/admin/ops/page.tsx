...
import { ENV_LAUNCH_CITY_CODE, ENV_NODE_ENV, ENV_SUPPORT_WHATSAPP_E164 } from "@/lib/env";
...
const OPS_MARKET_PROFILE = buildMarketFormattingProfile({ code: ENV_LAUNCH_CITY_CODE });

function formatMoney(value: number | null | undefined) {
  return formatMoneyForProfile(value, OPS_MARKET_PROFILE);
}

function formatDateTime(value: string | Date | undefined) {
  return formatDateTimeForProfile(value, OPS_MARKET_PROFILE);
}
...
export default async function AdminOpsPage({
  ...
  const devQueryKey =
    ENV_NODE_ENV !== "production" ? normalizeSingle(params.key).trim() : "";
  const key = headerAdminKey || devQueryKey;
  const hasAdminSession = Boolean(getAdminSessionFromCookieHeader(rawCookie));

  if (!key && !hasAdminSession) {
    return (
      <main className="mx-auto min-h-screen max-w-5xl p-6">
        <h1 className="text-2xl font-bold">Ops Center</h1>
        <p className="mt-2 text-sm text-red-600">
          Ops access requires a secure admin browser session.
        </p>
        <Link
          href="/admin/access?next=/admin/ops"
  ...
  const [
    ...
    await Promise.all([
      fetchJson<MaintenanceResponse>(`${baseUrl}/api/admin/maintenance`, adminRequestHeaders),
      fetchJson<MetricsResponse>(`${baseUrl}/api/admin/metrics`, adminRequestHeaders),
      fetchJson<SettlementAuditResponse>(`${baseUrl}/api/admin/audit?limit=5`, adminRequestHeaders),
      fetchJson<BusinessAuditResponse>(
        `${baseUrl}/api/admin/businesses/audit?limit=5`,
        adminRequestHeaders
      ),
      fetchJson<AtRiskResponse>(
        `${baseUrl}/api/admin/businesses/at-risk?limit=10`,
        adminRequestHeaders
      ),
      fetchJson<SettlementPreviewResponse>(
        `${baseUrl}/api/admin/settlement-previews?weekKey=${encodeURIComponent(
          currentWeekKey
        )}&mismatchOnly=true&limit=10`,
        adminRequestHeaders
      ),
      fetchJson<PromoSpendResponse>(
        `${baseUrl}/api/admin/promo-spend?weekKey=${encodeURIComponent(currentWeekKey)}&limit=10`,
        adminRequestHeaders
      ),
      fetchJson<ComplaintsResponse>(
        `${baseUrl}/api/admin/complaints?status=open&limit=20`,
        adminRequestHeaders
      ),
      fetchJson<OpenStatusResponse>(`${baseUrl}/api/admin/ops/open-status`, adminRequestHeaders),
      fetchJson<TrustOverviewResponse>(`${baseUrl}/api/admin/ops/trust`, adminRequestHeaders),
      fetchJson<AdminBusinessesResponse>(`${baseUrl}/api/admin/businesses`, adminRequestHeaders),
      fetchJson<MenuQualityResponse>(`${baseUrl}/api/admin/ops/menu-quality`, adminRequestHeaders),
      fetchJson<SearchTelemetryResponse>(
        `${baseUrl}/api/admin/ops/search-telemetry`,
        adminRequestHeaders
      ),
      fetchJson<FunnelResponse>(`${baseUrl}/api/admin/ops/funnel?days=7`, adminRequestHeaders),
      fetchJson<ReputationResponse>(
        `${baseUrl}/api/admin/ops/reviews?days=30&limit=100`,
        adminRequestHeaders
      ),
      fetchJson<BackupRunResponse>(`${baseUrl}/api/admin/backup-runs`, adminRequestHeaders),
      fetchJson<StatusResponse>(`${baseUrl}/api/status`),
    ]);
  const cashCollectionsReq = await fetchJson<CashCollectionResponse>(
    `${baseUrl}/api/admin/cash-collections?weekKey=${encodeURIComponent(currentWeekKey)}&limit=200`,
    adminRequestHeaders
  );
  const financeMismatchesReq = await fetchJson<FinanceMismatchResponse>(
    `${baseUrl}/api/admin/finance/mismatches?weekKey=${encodeURIComponent(
      currentWeekKey
    )}&limit=200`,
    adminRequestHeaders
  );
    ...
    <main className="mx-auto min-h-screen max-w-7xl p-6">
      ...
      <section className="mt-5 grid gap-4 lg:grid-cols-4">
        ...
        <article className="rounded-xl border border-slate-200 bg-white p-4">
          ...
          <div className="mt-3 flex flex-wrap gap-2">
            <a
              href={`/api/admin/settlements/export?weekKey=${encodeURIComponent(weekKey)}`}
      ...
      <section className="mt-5 rounded-xl border border-slate-200 bg-white p-4">
        ...
        <div className="mt-3 flex flex-wrap gap-2">
          ...
          <a
            href={`/api/admin/finance/export?weekKey=${encodeURIComponent(currentWeekKey)}`}
            target="_blank"
            rel="noreferrer"
            className="rounded-lg border border-slate-300 px-4 py-2 text-sm font-semibold"
          >
          ...
          <a
            href={`/admin/statements?weekKey=${encodeURIComponent(currentWeekKey)}`}
            target="_blank"
            rel="noreferrer"
            className="rounded-lg border border-slate-300 px-4 py-2 text-sm font-semibold"
          >
