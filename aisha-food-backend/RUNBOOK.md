...
### Toggle ON
Start the secure admin browser session from `/admin/access`, or use `x-admin-key: <ADMIN_KEY>` as a temporary API-only fallback.
...
For current week:
1. `GET /api/admin/settlements`
2. Confirm `ordersCount` and `feeTotal` changed as expected.
3. Spot-check audit:
   - `GET /api/admin/audit?businessId=...&weekKey=YYYY-Www&limit=100`

If payout review needed:
- Export CSV:
  - `GET /api/admin/settlements/export?weekKey=YYYY-Www`
...
### Manual Run (Admin)
- `POST /api/admin/jobs/backup-export`
- Optional payload:
```json
{
  "kind": "all",
