...
1. Open `/admin/access` and start a secure admin browser session, then open `/admin/ops`.
...
1. Open `/admin/businesses` after starting the secure admin session, or use the at-risk table in `/admin/ops`.
...
1. Open `/admin/settlements` after starting the secure admin session.
...
1. Pull weekly settlement CSV:
   - `/api/admin/settlements/export?weekKey=YYYY-Www`
2. Pull settlement audit trail:
   - `/api/admin/audit?businessId=<id>&weekKey=YYYY-Www&limit=200`
3. Pull business audit trail:
   - `/api/admin/businesses/audit?businessId=<id>&limit=200`
4. Cross-check:
...
5. If you need a temporary non-browser fallback, send `x-admin-key: <ADMIN_KEY>` instead of putting the key in the URL.
