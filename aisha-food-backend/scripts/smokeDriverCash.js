...
async function main() {
  ...
  const orderCoords = coordsAtDistance(city.centerLat, city.centerLng, 1.1);
  ...
  const driverCreate = await requestJson(`/api/admin/drivers?key=${encodeURIComponent(adminKey)}`, {
    ...
    body: {
      ...
      phoneE164: "+22370007203",
      zoneLabel: "Bamako Cash Zone",
      isActive: true,
      cityId: city.id,
    },
  });
  ...
  const firstDelivered = await requestJson(`/api/driver/orders/delivered?token=${encodeURIComponent(driverToken)}`, {
    method: "POST",
    body: { orderId, cashCollected: true, deliveryOtp },
  });
