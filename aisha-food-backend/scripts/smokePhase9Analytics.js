...
async function createRestaurantFixture(city) {
  ...
  const productCreate = await request("/api/merchant/products", {
    ...
    cookie: merchantCookie,
    body: {
      name: randomLabel("P9-AN-PROD"),
      price: 500,
      category: "Phase 9",
      description: "Analytics smoke item",
      isAvailable: true,
    },
  });
...
async function main() {
  console.log(`Running Phase-9 analytics smoke against ${baseUrl}`);
  ...
  const restaurants = await listRestaurants(String(selectedCity._id), "P9-AN-BIZ");
  const restaurant =
    restaurants.find((row) => String(row.restaurantId) === String(fixture.businessId)) || restaurants[0];
  assert(restaurant?.restaurantId, "Could not locate analytics fixture restaurant.");
  const menu = await loadMenu(String(selectedCity._id), String(restaurant.slug));
  const firstItem = Array.isArray(menu.menu) ? menu.menu[0] : null;
  assert(firstItem?.itemId, "Expected a menu item for analytics smoke.");
  ...
  const orderCreate = await request("/api/public/orders", {
    ...
    body: {
      ...
      customerName: "Phase9 Analytics Customer",
      phone: randomPhone(cityPhonePrefix(selectedCity)),
      address: `${selectedCity.name || "City"} analytics smoke address`,
      notes: "phase9 analytics smoke",
    },
  });
  ...
  assert(orderId, "Missing orderId from analytics smoke order.");
  assert(deliveryOtp, "Missing delivery OTP from analytics smoke order.");
  ...
  const [overview, sales] = await Promise.all([
    request("/api/merchant/analytics/overview?range=30d", {
      cookie: fixture.merchantCookie,
    }),
    request("/api/merchant/analytics/sales?range=30d", {
      cookie: fixture.merchantCookie,
    }),
  ]);
  assert(overview.res.ok && overview.json?.ok, `Analytics overview failed: ${overview.text}`);
  assert(sales.res.ok && sales.json?.ok, `Analytics sales failed: ${sales.text}`);

  const revenueTotal = Number(overview.json?.revenueTotal || 0);
  const ordersTotal = Number(overview.json?.ordersTotal || 0);
  assert(revenueTotal > 0, "Expected analytics revenueTotal to be greater than 0.");
  assert(ordersTotal > 0, "Expected analytics ordersTotal to be greater than 0.");
