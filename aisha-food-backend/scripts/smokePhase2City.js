...
async function runEnabledSuite(db, bamakoCity) {
  ...
  async function assertQuote(km, expectedFee, expectedPayout, expectedBandLabel) {
    ...
    if (!quote.res.ok) {
      throw new Error(
        `Quote failed at ${km}km (status=${quote.res.status}): ${JSON.stringify(quote.json)}`
      );
    }
...
async function main() {
  ...
  let restoreBamakoSnapshot = null;

  try {
    if (mode === "enabled" || mode === "enabled-flip") {
      restoreBamakoSnapshot = {
    ...
    if (restoreBamakoSnapshot) {
      await db
        .collection("cities")
        .updateOne({ _id: bamakoId }, { $set: restoreBamakoSnapshot });
    }
    if (flipServer) {
