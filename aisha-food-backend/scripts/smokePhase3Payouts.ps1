...
param(
  ...
  [ValidateSet("enabled", "enabled-flip")]
  [string]$Mode = "enabled",
...
Write-Host "Running Phase-3 payouts smoke against $normalizedBase (mode=$Mode) ..." -ForegroundColor Cyan
& node scripts/smokePhase3Payouts.js "--mode=$Mode"
if ($LASTEXITCODE -ne 0) {
  exit $LASTEXITCODE
}

Write-Host "Phase-3 payouts smoke completed successfully." -ForegroundColor Green
